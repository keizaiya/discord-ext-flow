...
def _result_finishes_pagination(result: Result) -> bool:
    """Cancel pending page modals on model transitions or terminal results.

    Page navigation within the same model keeps modals alive. ``disable_items`` only configures cleanup of the
    displayed message; it does not finish pagination or change the modal's model-scoped lifetime.
    Any explicit model result ends this pagination session, even if the target Model compares equal.
    """
    ...
    return not message.items or not items_can_produce_result(message.items)
...
class Paginator[T](_BasePaginator[T, LegacyMessage]):
    ...
    Pending page modals survive page navigation within the same Model, including when ``disable_items=True``.
    Any model transition returned by a paginator callback, including to an equal Model, cancels them.
    Other model changes and flow termination also cancel them through their Model lifetime.
