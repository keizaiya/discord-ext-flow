...
class Controller:
    ...
    This class owns model transitions, task scheduling, and error handling for one invocation. The internal
    :class:`FlowDisplay` owns the delivery context, active message and view, and disabling controls.
    Prefer :func:`run_flow` unless direct access is needed for external task registration or subclassing.
    Callers must use a fresh controller for each invocation; reuse is not checked. ``Controller.copy()`` has been
    removed; use :func:`run_flow` or construct a new controller for each run.

    A screen without enabled flow callbacks is terminal, even with pending external tasks. After sending it,
    the flow stops and cancels those tasks. Empty items are sent without a view; non-empty static layouts still
    create a view for rendering. Both initial and replacement screens follow this rule. Static layouts finish
    without waiting for a timeout, regardless of the configured timeout.

    UI callbacks and external result coroutines should handle expected errors themselves and return a :class:`Result`.
    For unhandled errors, the model's optional ``on_error(ExceptionGroup[Exception])`` handler takes precedence for
    that model's UI callback errors and view timeouts. The controller handler receives the remaining UI errors,
    external task errors, and timeouts, including modal timeouts.

    Error handlers receive original exceptions grouped in an ``ExceptionGroup``, with :class:`FlowTimeoutError`
    representing a view or modal timeout. No UI item or interaction is supplied. Handlers may return a :class:`Result`
    to recover or transition, or ``None`` to report only. Returning a replacement screen or a model transition can
    recover from a view timeout; ``None`` and :meth:`Result.continue_flow` let the expired view finish the flow.
    See :meth:`invoke` for cleanup-time error handling.
    ...
    async def invoke(self, messageable: Sendable, message: PartialMessage | None = None) -> None:
        ...
        A normal Messageable remains the destination for external non-ephemeral results throughout the flow,
        including after UI-driven model transitions and message replacements. External ephemeral sends and flows
        started from an interaction use the retained initial or component interaction. Modal submit interactions
        serve only their own result response and previous-screen cleanup, not subsequent external results.
        Ephemeral delivery requires an available interaction; internal helpers assert this precondition.

        For long-lived external notifications and edits, start the flow from a normal Messageable and use
        non-ephemeral messages. Their edits use bot authentication and remain possible after interaction token
        expiry, provided the bot can still access and edit the message. Ephemeral delivery stays ephemeral and
        requires a usable interaction token.

        Cleanup reclaims local views and tasks even if disabling a message fails. Exceptions recovered while
        reclaiming tasks propagate to the caller. Results from handlers notified during cleanup are not applied.

        Args:
            messageable (Sendable): Messageable or interaction to send first message.
            message (PartialMessage | None): The first target for editing if edit_original is True. Defaults to None.

        Note:
            Direct editing of an explicit target different from the interaction's source message does not
            acknowledge the interaction response. This also applies when the interaction has no source message;
            the caller must acknowledge the interaction before invoking in these cases.
        """
    ...
    async def _run_model(self) -> None:
        ...
        if self._display.view is not None:
            await self._retire_view(self._display.view)
        await maybe_coroutine(self.model.after_invoke)
    ...
    async def _send_and_activate_message(
        ...
        if view is not None and view_can_produce_result(view):
    ...
    async def _apply_result(self, result: Result) -> _ResultAction:
        ...
        match result._type:
            case _ResultTypeEnum.MESSAGE:
                ...
                if replacement is None or not view_can_produce_result(replacement):
                    return _ResultAction.FINISH_FLOW
                return _ResultAction.REPLACE_VIEW
    ...
    async def _apply_completed_result(
        ...
        if record.task not in self._tasks:
            return outcome, ()
        if outcome is _ResultAction.TRANSITION_MODEL and record.source is not None:
            model_changed = self._next_model != self.model
    ...
    async def _wait_on_view(self) -> _ResultAction:
        ...
        while True:
            if not view_can_produce_result(view):
                view.stop()
                return _ResultAction.FINISH_FLOW
    ...
    def on_error(self, error: ExceptionGroup[Exception]) -> MaybeAwaitable[Result | None]:
        ...
        the flow, or ``None`` to only report the errors. The default implementation logs the group and returns
        ``None``. An ``on_error`` callback supplied to the constructor takes precedence over this method.
        See :class:`Controller` for handler precedence, timeout recovery, and the error group contract.
        """
...
async def run_flow(
    ...
    *,
    ...
    This is the recommended entry point: it constructs a :class:`Controller` and awaits :meth:`Controller.invoke`.
    Use a controller directly when external task registration or subclassing requires access to it.

    Example:
        Start a flow with an initial model and an error handler::
