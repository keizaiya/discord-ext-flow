...
class _ReplacingExternalTaskV2Model(ModelBase):
    ...
    def message(self) -> ComponentV2Message:
        return ComponentV2Message(items=(ActionRow(items=(Button(label='Wait').on(callback=_callback),)),))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (TextDisplay('Component V2 content'), ActionRow(items=(Button().on(callback=_callback),))),
    ],
)
@pytest.mark.asyncio
async def test_view_types_complete_through_flow_invoke(
    ...
    if isinstance(items[0], TextDisplay):
        assert isinstance(view, _LayoutView)
        await view.children[1].children[0].callback(_interaction())  # type: ignore[attr-defined]
    else:
        assert isinstance(view, _View)
...
@pytest.mark.asyncio
async def test_static_v2_model_finishes_with_registered_external_task(monkeypatch: pytest.MonkeyPatch) -> None:
    """A static layout finishes without applying a registered external transition."""
    send = create_autospec(FlowDisplay._send, return_value=_sent())
    monkeypatch.setattr(FlowDisplay, '_send', send)
    ...
    assert controller._display._interaction is None
    assert not terminal_model.after_invoked
    assert not controller._tasks


@pytest.mark.asyncio
async def test_static_message_replacement_cancels_pending_external_task(monkeypatch: pytest.MonkeyPatch) -> None:
    """A static replacement finishes the flow and cancels a pending external transition."""
...
@pytest.mark.asyncio
async def test_static_message_replacement_cancels_newly_registered_successor(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A successor registered by the replacing task is also cancelled on a static screen."""
    replacement_sent = asyncio.Event()
...
@pytest.mark.asyncio
async def test_static_v2_model_reports_external_failure_already_completed(monkeypatch: pytest.MonkeyPatch) -> None:
    """Static flow cleanup reports an external failure that completed before delivery."""
    send = create_autospec(FlowDisplay._send, return_value=_sent())
    monkeypatch.setattr(FlowDisplay, '_send', send)
    ...
    with pytest.raises(RuntimeError, match='external task failed'):
        await asyncio.wait_for(controller.invoke(_messageable()), timeout=0.1)
...
async def test_nested_v2_button_returns_result_to_flow() -> None:
    ...
    view = create_view({}, (layout,), Controller(_Model()))
...
async def test_deferred_ephemeral_v2_response_retains_interaction_message(
    ...
    response_message = MagicMock(
        spec=DiscordMessage,
        flags=SimpleNamespace(ephemeral=True, loading=True),
        delete=AsyncMock(),
    )
    response = SimpleNamespace(
        is_done=lambda: True,
    ...
    loading_check = AsyncMock(return_value=True)
    monkeypatch.setattr(display, '_deferred_original_is_loading', loading_check)
    ...
    loading_check.assert_awaited_once_with(interaction)
