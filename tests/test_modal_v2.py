...
async def test_modal_timeout_does_not_cancel_a_submit_callback_in_progress() -> None:
    ...
    await inner_modal.on_submit(interaction)
    await started.wait()
...
@pytest.mark.asyncio
async def test_modal_submit_preserves_submission_context_and_controller() -> None:
    """A modal opened by a Select reads its own submission when both use the same custom ID."""
    controller = Controller(ModelBase())
    request_context = ContextVar('request_context', default='outside')
    selected = flow.Select(custom_id='choice', options=(flow.SelectOption(label='new'),)).field()
    received: list[tuple[Controller, str, list[str]]] = []
    waiters: list[asyncio.Task[Result]] = []

    async def callback(_: Interaction[Client]) -> Result:
        await asyncio.sleep(0)
        received.append((_get_controller(), request_context.get(), selected.value))
        return Result.finish_flow()

    class OpeningSelect(ui.Select[ui.View]):
        async def callback(self, interaction: Interaction[Client]) -> None:
            token = request_context.set('opening')
            try:
                task = await modal.send_modal(
                    callback,
                    interaction,
                    modal.ModalConfig(title='Context'),
                    (flow.Label(text='Choice', component=selected),),
                    controller=controller,
                )
                waiters.append(task.task)
            finally:
                request_context.reset(token)

    source = OpeningSelect(custom_id='choice', options=[flow.SelectOption(label='old')])
    view = ui.View(timeout=None)
    view.add_item(source)
    send_response = AsyncMock()
    send_interaction = _send_modal_interaction(send_response)
    send_interaction.data = {'custom_id': 'choice', 'component_type': 3, 'values': ['old']}
    await asyncio.create_task(view._scheduled_task(source, send_interaction))
    view.stop()
    assert send_response.await_args is not None
    sent = send_response.await_args.args[0]
    assert isinstance(sent, _InnerModal)
    submit_interaction = _interaction()

    async def submit() -> None:
        token = request_context.set('submission')
        try:
            await sent._scheduled_task(
                submit_interaction,
                [{'type': 18, 'component': {'type': 3, 'custom_id': 'choice', 'values': ['new']}}],
                {},
            )
        finally:
            request_context.reset(token)
    ...
    assert received == [(controller, 'submission', ['new'])]
