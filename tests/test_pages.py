...
@pytest.mark.asyncio
async def test_page_navigation_keeps_modal_until_model_transition() -> None:
    """Cleanup settings on page messages do not cancel modals; leaving the model does."""
    task = _ModalTask()
    next_model = _Model()

    def transition(_: Interaction) -> Result:
        return Result.next_model(next_model)

    def builder(values: tuple[int, ...], _: int, __: int, controls: PaginatorControls) -> ComponentV2Message:
        return ComponentV2Message(
            items=(
                TextDisplay(str(values)),
                ActionRow(items=controls),
                ActionRow(items=(Button(label='Leave').on(callback=transition),)),
            ),
            disable_items=True,
        )
    ...
    row = message.items[1]
    assert isinstance(row, ActionRow)
    next_page = row.items[3]
    assert _is_interactive_button(next_page)
    result = await maybe_coroutine(next_page.callback, _interaction())
    assert pagination.current_page == 1
    assert not task.cancelled
    assert isinstance(result._message, ComponentV2Message)
    leave_row = result._message.items[2]
    assert isinstance(leave_row, ActionRow)
    leave = leave_row.items[0]
    assert _is_interactive_button(leave)
    result = await maybe_coroutine(leave.callback, _interaction())
    assert result._model is next_model
    assert task.cancelled


@pytest.mark.parametrize('v2', [False, True])
@pytest.mark.asyncio
async def test_paginator_callback_cancels_page_modal_for_same_model(v2: bool) -> None:
    """Explicit paginator model results cancel page modals even when ordinary MODEL tasks would survive."""
    model = _Model()
    controller = Controller(model)
    task = _ModalTask()
    refresh = Button(label='Refresh').on(callback=lambda _: Result.next_model(model))

    def legacy_builder(_: tuple[int, ...], __: int, ___: int) -> LegacyMessage:
        return LegacyMessage(content='Page', items=(refresh,))

    def v2_builder(_: tuple[int, ...], __: int, ___: int, controls: PaginatorControls) -> ComponentV2Message:
        return ComponentV2Message(items=(ActionRow(items=(refresh,)), ActionRow(items=controls)))

    pagination = ComponentV2Paginator(v2_builder, range(20)) if v2 else Paginator(legacy_builder, range(20))
    pagination.modal_tasks.append(task)
    message = await pagination._message()
    view = create_view({}, message.items or (), controller)
    button = next(item for item in view.walk_children() if isinstance(item, ui.Button) and item.label == 'Refresh')
    await button.callback(_interaction())
    record = next(iter(controller._tasks.values()))
    result = await record.task
    assert isinstance(result, Result)
    assert result._model is controller.model
    assert task.cancelled
