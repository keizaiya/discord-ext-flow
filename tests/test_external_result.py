...
async def test_unprocessed_persistent_results_continue_into_next_models(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class InitialModel(ModelBase):
        ...
        async def message(self) -> ComponentV2Message:
            ...
            return ComponentV2Message(items=(ActionRow(items=(Button().on(callback=_finish),)),))
...
@pytest.mark.parametrize('replacements', [0, 1, 2])
@pytest.mark.parametrize('same_model', [False, True])
@pytest.mark.asyncio
async def test_active_view_callbacks_are_reclaimed_before_model_hooks(  # noqa: C901 - Check hooks across view and model transitions.
    monkeypatch: pytest.MonkeyPatch,
    v2: bool,
    replacements: int,
    same_model: bool,
) -> None:
    """Retire the latest view before hooks, including tasks registered by callback cleanup."""
    callback_started = asyncio.Event()
    callback_closed = asyncio.Event()
    views: list[_ViewType] = []
    cleanup_tasks: list[ExternalResultTask] = []
    cleanup_models: list[ModelBase] = []
    hooks: list[str] = []

    async def pending_result() -> Result:
        await asyncio.Event().wait()
        return Result.continue_flow()

    async def pending_callback(_: Interaction) -> Result:
        callback_started.set()
        try:
            return await pending_result()
        finally:
            cleanup_models.append(controller.model)
            cleanup_tasks.append(create_external_result(pending_result, life_time=ExternalTaskLifeTime.MODEL))
            callback_closed.set()

    def check_retirement() -> None:
        assert callback_closed.is_set()
        assert all(view.is_finished() for view in views)
        assert cleanup_models == [initial_model]

    class NextModel(ModelBase):
        def before_invoke(self) -> None:
            check_retirement()
            assert cleanup_tasks[0].task.cancelled()
            hooks.append('next-before')

        def message(self) -> LegacyMessage:
            return LegacyMessage(content='Finished')

    next_model = NextModel()

    def screen(stage: int) -> LegacyMessage | ComponentV2Message:
        if stage < replacements:
            items = (Button().on(callback=lambda _: Result.send_message(screen(stage + 1))),)
        else:
            target = initial_model if same_model else next_model
            items = (
                Button().on(callback=pending_callback),
                Button().on(callback=lambda _: Result.next_model(target)),
            )
        return ComponentV2Message(items=(ActionRow(items=items),)) if v2 else LegacyMessage(items=items)
    ...
    class InitialModel(ModelBase):
        ...
        def before_invoke(self) -> None:
            ...
            if self.runs == 2:
                check_retirement()
                assert not cleanup_tasks[0].task.cancelled()
                hooks.append('same-before')

        def message(self) -> LegacyMessage | ComponentV2Message:
            return screen(0) if self.runs == 1 else LegacyMessage(content='Finished')

        def after_invoke(self) -> None:
            check_retirement()
            hooks.append('after')

    async def send(_: FlowDisplay, __: LegacyMessage | ComponentV2Message, view: _ViewType | None) -> PartialMessage:
        if view is not None:
            views.append(view)
            buttons = [
                cast('ui.Button[_ViewType]', item) for item in view.walk_children() if isinstance(item, ui.Button)
            ]
            await buttons[0].callback(_interaction())
            if len(views) == replacements + 1:
                await callback_started.wait()
                await buttons[1].callback(_interaction())
        return _sent()
    ...
    await asyncio.wait_for(controller.invoke(_messageable()), timeout=1)

    assert hooks == (['after', 'same-before', 'after'] if same_model else ['after', 'next-before'])
    assert len(views) == replacements + 1
    assert cleanup_tasks[0].task.cancelled()
    assert not controller._tasks


@pytest.mark.parametrize('life_time', list(ExternalTaskLifeTime))
@pytest.mark.parametrize('completed', [False, True], ids=['pending', 'completed'])
@pytest.mark.parametrize('transition_source', ['ui', 'external'])
@pytest.mark.asyncio
async def test_same_model_transition_preserves_external_finish_result(  # noqa: C901 - Exercise task timing and transition sources.
    monkeypatch: pytest.MonkeyPatch,
    life_time: ExternalTaskLifeTime,
    completed: bool,
    transition_source: str,
) -> None:
    """Same-model transitions retain external results regardless of completion timing or lifetime."""
    release = asyncio.Event()
    external_tasks: list[ExternalResultTask] = []
    hooks: list[str] = []
    ...
    class Model(ModelBase):
        ...
        def before_invoke(self) -> None:
            ...
            hooks.append('before')
            if self.runs == 1:
                if transition_source == 'external':
                    external_tasks.append(create_external_result(lambda: Result.next_model(self)))

                async def finish() -> Result:
                    await release.wait()
                    return Result.finish_flow()

                external_tasks.append(create_external_result(finish, life_time=life_time))
                if completed:
                    release.set()
            else:
                assert self.runs == 2
                assert not external_tasks[-1].task.cancelled()
                release.set()

        async def message(self) -> LegacyMessage:
            if self.runs == 1 and completed:
                await asyncio.gather(*(task.task for task in external_tasks))
            return LegacyMessage(items=(Button().on(callback=lambda _: Result.next_model(self)),))

        def after_invoke(self) -> None:
            hooks.append('after')

    async def send(_: FlowDisplay, __: LegacyMessage, view: _ViewType) -> PartialMessage:
        if model.runs == 1 and transition_source == 'ui':
            await _first_button(view).callback(_interaction())
            # Finish the callback before returning so it shares the batch with completed external results.
            await asyncio.gather(*(task for task in controller._tasks if task.get_name() == 'flow-ui-callback'))
        return _sent()
    ...
    assert external_tasks[-1].result() == Result.finish_flow()
    assert not controller._tasks
...
@pytest.mark.parametrize('completed', [False, True], ids=['pending', 'completed'])
@pytest.mark.asyncio
async def test_different_model_transition_discards_only_model_tasks(
    monkeypatch: pytest.MonkeyPatch,
    life_time: ExternalTaskLifeTime,
    completed: bool,
) -> None:
    """A real model change still drops MODEL tasks and carries PERSISTENT tasks forward."""
    release = asyncio.Event()
    external_tasks: list[ExternalResultTask] = []

    class NextModel(_InteractiveModel):
        def before_invoke(self) -> None:
            task = external_tasks[-1].task
            assert (task in controller._tasks) is (life_time is ExternalTaskLifeTime.PERSISTENT)
            if life_time is ExternalTaskLifeTime.MODEL:
                assert task.cancelled() is not completed
                create_external_result(Result.finish_flow)
            release.set()
    ...
    class InitialModel(ModelBase):
        def before_invoke(self) -> None:
            ...
            external_tasks.append(create_external_result(lambda: Result.next_model(next_model)))
            external_tasks.append(create_external_result(finish, life_time=life_time))
            if completed:
                release.set()

        async def message(self) -> LegacyMessage:
            tasks = external_tasks if completed else external_tasks[:1]
...
@pytest.mark.parametrize('completed', [False, True], ids=['pending', 'completed'])
@pytest.mark.asyncio
async def test_equal_model_then_different_model_reclaims_model_tasks(
    monkeypatch: pytest.MonkeyPatch,
    life_time: ExternalTaskLifeTime,
    completed: bool,
) -> None:
    """Tasks survive an equal instance and are reclaimed on the next unequal model."""
    release = asyncio.Event()
    external_tasks: list[ExternalResultTask] = []
    visited: list[ModelBase] = []
    ...
    class Model(ModelBase):
        ...
        def before_invoke(self) -> None:
            ...
            if len(visited) == 1:
                ...
                task = external_tasks[0].task
                retained = self.value == 1 or life_time is ExternalTaskLifeTime.PERSISTENT
                assert (task in controller._tasks) is retained
                assert task.cancelled() is (not retained and not completed)

        async def message(self) -> LegacyMessage:
            if completed:
                await external_tasks[0].task
            if self.value == 2:
                return LegacyMessage(content='Finished')
            target = equal_model if len(visited) == 1 else different_model
            return LegacyMessage(items=(Button().on(callback=lambda _: Result.next_model(target)),))

    async def send(_: FlowDisplay, __: LegacyMessage, view: _ViewType | None) -> PartialMessage:
        if view is not None:
            await _first_button(view).callback(_interaction())
            # Process the transition before external results already completed in this batch.
            await asyncio.gather(*(task for task in controller._tasks if task.get_name() == 'flow-ui-callback'))
        return _sent()

    initial_model, equal_model, different_model = Model(1), Model(1), Model(2)
    monkeypatch.setattr(FlowDisplay, '_send', send)
    controller = Controller(initial_model)
    ...
    assert visited[0] is initial_model
    assert visited[1] is equal_model
    assert visited[2] is different_model
    assert not controller._tasks
...
async def test_view_model_result_precedes_simultaneous_external_message(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    interaction = _interaction()
    next_message = LegacyMessage(content='Next model', items=(Button().on(callback=_finish),))
...
async def test_modal_timeout_releases_its_external_result(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    controller = Controller(_StaticModel(), on_error=on_error)
...
async def test_external_task_name_cannot_collide_with_view_waiter(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class CollisionModel(_InteractiveModel):
        ...
        def before_invoke(self) -> None:
            async def replace_message() -> Result:
                return Result.send_message(
                    ComponentV2Message(items=(ActionRow(items=(Button().on(callback=_finish),)),)),
